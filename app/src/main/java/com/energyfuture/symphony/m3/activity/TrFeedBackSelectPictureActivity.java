...
public class TrFeedBackSelectPictureActivity extends Activity {
    ...
    private ArrayList<String> selectedPicture2 = new ArrayList<String>();
//    private String cameraPath = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.communion_select_picture);
        ...
        initView();
        for(String key: Utils.picCheckedFeedBack.keySet()){
            selectedPicture2.add(key);
        }
    }
...
//            Utils.picCheckedFeedBack.clear();
            for(String path:selectedPicture){
                Utils.picCheckedFeedBack.put(path,"true");
            }
        }
        selectedPicture.clear();
        for (String key : Utils.picCheckedFeedBack.keySet()) {
            selectedPicture.add(key);
        }
    ...
    private void initView() {
        ...
        btn_ok.setText("完成  "+ Utils.picCheckedFeedBack.size()+"/" + MAX_NUM);
    ...
    protected void goCamare() {
        if (Utils.picCheckedFeedBack.size() + 1 > MAX_NUM) {
...
//        Utils.cameraPath = mediaFile.getAbsolutePath();
        Utils.cameraPath.add(mediaFile.getAbsolutePath());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && Utils.cameraPath != null) {
            for(String path: Utils.cameraPath){
                selectedPicture.add(path);
            }
//            selectedPicture.add(Utils.cameraPath);
//            Utils.picCheckedFeedBack.put(cameraPath,"true");
            if(selectedPicture.size() > 0){
                for(String path:selectedPicture){
                    Utils.picCheckedFeedBack.put(path,"true");
                }
    ...
    class PictureAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                convertView = View.inflate(context, R.layout.communion_select_picture_item, null);
            ...
            if (position == 0) {
                ...
                if(Utils.picCheckedFeedBack.get(item.path) != null && !Utils.picCheckedFeedBack.get(item.path).equals("")){
                    holder.checkBox.setBackgroundResource(R.drawable.comm_pic_select);
                    holder.iv.setColorFilter(Color.parseColor("#77000000"));
                }else{
                    holder.checkBox.setBackgroundResource(R.drawable.comm_pic_unselect);
                    holder.iv.setColorFilter(Color.parseColor("#00000000"));
                }
                final ViewHolder finalHolder = holder;
                holder.iv.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (Utils.picCheckedFeedBack.containsKey(item.path)) {
                            selectedPicture.remove(item.path);
                            Utils.picCheckedFeedBack.remove(item.path);
                            finalHolder.checkBox.setBackgroundResource(R.drawable.comm_pic_unselect);
                            finalHolder.iv.setColorFilter(Color.parseColor("#00000000"));
                        } else {
                            if (!v.isSelected() && Utils.picCheckedFeedBack.size() + 1 > MAX_NUM) {
                                Toast.makeText(context, "最多选择" + MAX_NUM + "张", Toast.LENGTH_SHORT).show();
                                return;
                            }
                            selectedPicture.add(item.path);
                            Utils.picCheckedFeedBack.put(item.path,"true");
                            finalHolder.checkBox.setBackgroundResource(R.drawable.comm_pic_select);
                            finalHolder.iv.setColorFilter(Color.parseColor("#77000000"));
                        }
                        btn_ok.setText("完成  " + Utils.picCheckedFeedBack.size() + "/" + MAX_NUM);
...
//                holder.checkBox.setSelected(isSelected);
    ...
    class FolderAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                convertView = View.inflate(context, R.layout.communion_list_dir_item, null);
    ...
    public void onBackPressed() {
        ...
        Utils.picCheckedFeedBack.clear();
        for(String path:selectedPicture2){
            Utils.picCheckedFeedBack.put(path,"true");
        }
    }
}

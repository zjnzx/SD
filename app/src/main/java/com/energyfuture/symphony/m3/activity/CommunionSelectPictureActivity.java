...
public class CommunionSelectPictureActivity extends Activity {
...
//    private String cameraPath = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initView();
        for(String key: Utils.picChecked.keySet()){
...
//            Utils.picChecked.clear();
            for(String path:selectedPicture){
                Utils.picChecked.put(path,"true");
            }
        }
    ...
    private void initView() {
        ...
        btn_ok.setText("完成  "+ Utils.picChecked.size()+"/" + MAX_NUM);
    ...
    protected void goCamare() {
        if (Utils.picChecked.size() + 1 > MAX_NUM) {
...
//            Utils.picChecked.put(cameraPath,"true");
            if(selectedPicture.size() > 0){
                for(String path:selectedPicture){
                    Utils.picChecked.put(path,"true");
                }
    ...
    class PictureAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (position == 0) {
                ...
                if(Utils.picChecked.get(item.path) != null && !Utils.picChecked.get(item.path).equals("")){
                ...
                holder.iv.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (Utils.picChecked.containsKey(item.path)) {
                            selectedPicture.remove(item.path);
                            Utils.picChecked.remove(item.path);
                            ...
                            if (!v.isSelected() && Utils.picChecked.size() + 1 > MAX_NUM) {
                                Toast.makeText(context, "最多选择" + MAX_NUM + "张", Toast.LENGTH_SHORT).show();
                                return;
                            }
                            selectedPicture.add(item.path);
                            Utils.picChecked.put(item.path,"true");
